...
def handleSignup(request):
    if request.method =='POST':
            ...
            profile = UserProfile(Savings = Savings,profession=profession)
...
def stats(request):
    if request.session.has_key('is_logged') :
        todays_date = datetime.date.today()
        one_month_ago = todays_date -datetime.timedelta(days=365)
        user_id = request.session["user_id"]
        user1 = User.objects.get(id=user_id)
        ...
        x= addmoney_info.sum1 - addmoney_info.sum
        y= addmoney_info.sum1 - addmoney_info.sum
        addmoney_info.todays_date = todays_date
        if x<0:
            messages.warning(request,'Your expenses exceeded your income')
            x=0
        if x>0:
            y=0
...
def info_year(request):
    ...
    zz=1
    if xx<zz:
            messages.warning(request,'Your Savings in Current month is less than the Previous month')
    ...
    def get_expense_category_amount(Category,add_money):
        quantity = 0 
        filtered_by_category = addmoney.filter(Category = Category,add_money="Savings") 
        for item in filtered_by_category:
            quantity+=item.quantity
        return quantity
    ...
    for x in addmoney:
        for y in Category_list:
            finalrep[y]= get_expense_category_amount(y,"Savings")
